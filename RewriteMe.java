...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category){
        return (int) questions.stream().filter(x -> x.category == category).count();
    }

    //Skapa en lista innehållandes samtliga frågesträngar i databasen
    public List<String> getListOfAllQuestions(){
        return questions.stream().map(x -> x.question).collect(Collectors.toList());
    }
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
        return questions.stream().filter(x -> x.category == category).map(x -> x.question).collect(Collectors.toList());
    }
    ...
    public List<String> getAllAnswerOptionsDistinct(){
        return questions.stream().flatMap(x -> x.answers.stream()).distinct().collect(Collectors.toList());
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate){
        return questions.stream().anyMatch(x -> x.answers.contains(answerCandidate));
    }
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate){
        return (int) questions.stream().filter(x -> x.answers.contains(answerCandidate)).count();
    }
    ...
    public Map<Category, List<String>> getQuestionGroupedByCategory(){
        return null;
//        return questions.stream().collect(groupingBy(question -> question));
    }
    ...
    {
        return questions.stream().filter(x -> x.category == c).flatMap(x -> x.answers.stream()).reduce("", (longest, answer) -> {
            if(answer.length() > longest.length()) return answer;
            return longest;
        });
    }
